...
const EditModal = () => {
  ...
  const [text1, setText1] = useState<string>(detailInform.inform);
  ...
  const onSubmit = async (e: any) => {
    ...
    if (regs.title === undefined) {
      regs.title = detailInform.title;
    }
    params.append("title", regs.title);
    params.append("inform", text1);
    params.append("date", reg_date);
    await axios
      .patch(`http://localhost:3001/informs/` + detailInform.id, params)
      .then(() => {
        getInforms();
        ...
        <Form className="form_react" onSubmit={onSubmit} onChange={onChange}>
          ...
          >
            ...
            <Form.Control
              name="title"
              type="text"
              placeholder={detailInform.title}
              maxLength={20}
